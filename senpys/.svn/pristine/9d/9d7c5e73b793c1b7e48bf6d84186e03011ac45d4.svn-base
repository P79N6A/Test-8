create or replace procedure pysBilVoiceL(szDate varchar2) is
...
(
(select tariffinfo,CALLDURIATION,CALLEDPARTYLOCATIONCITY,callingpartylocationcity,longdistancetype,roamingtype,payflag from cdr_cdr100_'||to_char(to_date(szDate,'yyyymm'),'mm')||'  a ,info_user b  where not call_type in(1,2,3) and  a.servid=b.user_id and  not b.tele_type=''CTC'' and longdistancetype in(''5'',''4'') and not a.roamingtype in (4,5) and payflag=0)
)
';
...
acct_item_code=v_tem;
      if v_free_flag<>1 then
          update pys_bil_cdr set bil_value=bil_value+ceil(v_callduriation)
...
(busi_type,busi_code,free_flag,longdistancetype,roamingtype,area_code,bil_value,sale_flag,insert_date)
              values(v_busi_type,v_busi_code,v_free_flag,v_longdist,v_roaming,v_area_code,ceil(v_callduriation),v_sale_flag,szDate);
           end if;
           exit;
      end if;
...
(busi_type,busi_code,free_flag,longdistancetype,roamingtype,area_code,bil_value,sale_flag,insert_date)
       ...
       v_callduriation:=v_callduriation-v_value;
