...
<script LANGUAGE="javascript"> 
	function submit10() {
		self.location.replace("ctexaPayable.jsp")
} 

function addCombineGroupAll()
{	
   location.href = "./ctexaPayable.jsp?up=up" ;  
   alert("审核完毕！");

}
function RCombineGroupAll()
{	
   location.href = "./ctexaPayable.jsp?rup=rup" ;  
   alert("审核完毕！");
...
<%
	...
	sqlStmt = sqlCon.createStatement(java.sql.ResultSet.TYPE_SCROLL_INSENSITIVE,
	   ...
	   if(upss!=null && !"".equals(upss)) {  
			sqlStmt.execute("update ctpayable set zt='已审核'");
		} 
	   ...
	   if(rup!=null && !"".equals(rup)) {  
			sqlStmt.execute("update ctpayable set zt='未审核'");
		}  


	strSQL = "select * from ctpayable";
	//执行SQL语句并获取结果集 
...
<body>
	...
	<table width="800" border="1" cellspacing="0" cellpadding="0">
		...
		<%
		out.println("<tr><td align=\"center\" bgcolor=\"#F6F6F6\" background=\"images/top_bg.gif\">序号</td><td align=\"center\" bgcolor=\"#F6F6F6\" background=\"images/top_bg.gif\">通话模式</td><td align=\"center\" bgcolor=\"#F6F6F6\" background=\"images/top_bg.gif\">月费（元/月）</td><td align=\"center\" bgcolor=\"#F6F6F6\" background=\"images/top_bg.gif\">呼叫类型</td><td align=\"center\" bgcolor=\"#F6F6F6\" background=\"images/top_bg.gif\">包资源量</td><td align=\"center\" bgcolor=\"#F6F6F6\" background=\"images/top_bg.gif\">被叫及超出后资费</td><td align=\"center\" bgcolor=\"#F6F6F6\" background=\"images/top_bg.gif\">状态</td><td align=\"center\" bgcolor=\"#F6F6F6\" background=\"images/top_bg.gif\"> </td></tr>");
			...
			if (intPageCount > 0) {
				...
				while (i < intPageSize && !sqlRst.isAfterLast()) {
					 ...
					 cos++;
					 String col1 = sqlRst.getString("call_mot");
					 String col2 = sqlRst.getString("mon_fee");
					 String col3 = sqlRst.getString("call_type");
					 String col4 = sqlRst.getString("bo_sale");
					 String col5 = sqlRst.getString("call_bak");
					 String col6 = sqlRst.getString("zt");
					 ids += "," + cos; 
		%>
		<tr>
	...
	%>
		<td><span class="font_blue"
			style="padding-top: 1px; font-weight: bold; line-height: 14px;"><a
				href="ctexaPayable.jsp?page=<%=intPage + 1%>">下一页</a></span> <%
			}
		%> <%
			if (intPage > 1) {
		%>
		<td><span class="font_blue"
			style="padding-top: 1px; font-weight: bold; line-height: 14px;"><a
				href="ctexaPayable.jsp?page=<%=intPage - 1%>">上一页</a></span> <%
			}
		%>
		<td><span class="font_blue"
