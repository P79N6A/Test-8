 ...
 * @version
 */
public class Ua2Driver {
	private static final Logger L = Logger.getLogger(Ua2Driver.class);
	...
	public Ua2Driver(String configDir, int threadCounts) throws Exception {
	...
	public void run() {
		...
		// 启动Type2Processer
		try {
			Type2Processer[] process = new Type2Processer[threadCounts];
			Thread[] processThread = new Thread[threadCounts];
			for (int i = 0; i < process.length; i++) {
				process[i] = new Type2Processer();
				...
				process[i].init();
				processThread[i] = new Thread(process[i], "Type2Processer-" + i);
			}
