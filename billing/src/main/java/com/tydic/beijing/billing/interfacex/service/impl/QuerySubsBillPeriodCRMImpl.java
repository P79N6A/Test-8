...
import com.tydic.beijing.billing.interfacex.service.QuerySubsBillPeriodCRM;

public class QuerySubsBillPeriodCRMImpl implements QuerySubsBillPeriodCRM {
	...
	public SubsBillPeriod query(QuerySubsBillPeriodInfo info) {
			...
			log.debug("服务[QuerySubsBillPeriodCRM],号码["+nbr+"],渠道["+channle+"]");
			...
			String nowyear=sdf.format(calnow.getTime()).substring(0,6);	//不含当前时间点
...
//					bill.setErrorCode("ZSMART-CC-00032");
				...
				for(CodeAcctMonth  month : acctMonths){
					...
					BillCycleDto cycle=new BillCycleDto(acct_month,effDate,expDate);
					dtos.add(cycle);
				...
				//LOG
				log.debug("服务[QuerySubsBillPeriodCRM],号码["+nbr+"],渠道["+channle+"] doQuery return :"+bill);
				for(BillCycleDto dto : dtos){
					log.debug("服务[QuerySubsBillPeriodCRM],号码["+nbr+"],渠道["+channle+"] doQuery return:"+dto);
				}
				return bill;
